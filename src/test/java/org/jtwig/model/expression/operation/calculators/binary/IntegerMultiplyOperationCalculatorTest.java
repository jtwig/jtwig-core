...
public class IntegerMultiplyOperationCalculatorTest {
    ...
    public void integerMultiplyWithFloating() throws Exception {
        ...
        when(leftOperand.calculate(renderContext)).thenReturn(JtwigValueFactory.value(new BigDecimal("1.3"), NamedValueConfiguration.COMPATIBLE_MODE));
        when(rightOperand.calculate(renderContext)).thenReturn(JtwigValueFactory.value(new BigDecimal("2.3"), NamedValueConfiguration.COMPATIBLE_MODE));
