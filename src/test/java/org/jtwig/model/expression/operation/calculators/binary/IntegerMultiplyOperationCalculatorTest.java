...
import org.jtwig.model.expression.operation.binary.calculators.IntegerMultiplyOperationCalculator;
...
public class IntegerMultiplyOperationCalculatorTest {
    ...
    public void integerMultiplyWithFloating() throws Exception {
        ...
        when(leftOperand.calculate(renderContext)).thenReturn(JtwigValueFactory.value(new BigDecimal("1.3"), VALUE_ENVIRONMENT));
        when(rightOperand.calculate(renderContext)).thenReturn(JtwigValueFactory.value(new BigDecimal("2.3"), VALUE_ENVIRONMENT));
